...
export default function Effective() {
    ...
    const filteredData = useMemo(() => {
        // Pastikan loanRecognitions adalah array
        if (!Array.isArray(loanRecognitions)) {
            console.warn('loanRecognitions is not an array:', loanRecognitions);
            return [];
        }
        return loanRecognitions.filter(item => {
            ...
            .then((XLSX) => {
                ...
                const excelData = [
                    ...
                    ['No', 'Entity Number', 'Account Number', 'Debitor Name', 'GL Account',
                     ...
                     'EIR Calculated Convertion', 'EIR Calculated Transaction Cost', 'EIR Calculated UpFront Fee',
                     'Outstanding Amount', 'Outstanding Amount Initial Transaction Cost', 'Outstanding Amount Initial UpFront Fee'],
                    // Data rows
                    ...filteredData.map((item, index) => [
                        ...
                        Number(item.original_balance || 0),
                        ...
                        Number(item.eir_calculated_convertion || 0),
                        Number(item.eir_calculated_transaction_cost || 0),
                        Number(item.eir_calculated_up_front_fee || 0),
                        Number(item.outstanding_amount || 0),
                        Number(item.outstanding_amount_initial_transaction_cost || 0),
                        Number(item.outstanding_amount_initial_up_front_fee || 0)
                    ])
                ];
                ...
                ws['!cols'] = [
                    ...
                    { width: 20 },  // EIR Calculated Convertion
                    { width: 25 },  // EIR Calculated Transaction Cost
                    { width: 22 },  // EIR Calculated UpFront Fee
                    { width: 18 },  // Outstanding Amount
                    { width: 32 },  // Outstanding Amount Initial Transaction Cost
                    { width: 30 }   // Outstanding Amount Initial UpFront Fee
                ];
                ...
                XLSX.utils.book_append_sheet(wb, ws, 'Effective Interest Rate Data');
                
                // Generate filename
                const filename = `effective_interest_rate_data_${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    const exportToPDF = () => {
        ...
        const htmlContent = `
            ...
            <html>
            <head>
                <title>Effective Interest Rate Report</title>
                <style>
            ...
            <body>
                <div class="header-info">
                    <h1>Report Loan - Report Initial Recognition</h1>
                    ...
                    <tbody>
                        ${filteredData.map((item, index) => `
                            <tr>
                                ...
                                <td>Rp ${Number(item.original_balance || 0).toLocaleString('id-ID')}</td>
                                ...
                                <td>Rp ${Number(item.outstanding_amount || 0).toLocaleString('id-ID')}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Report Loan - Report Initial Recognition - Effective Interest Rate" />
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                {/* Header dengan judul dan tombol aksi */}
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Report Loan - Report Initial Recognition</h1>
                    ...
                    <CardContent className="pt-6">
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="text-sm text-muted-foreground">
                                {filteredData.length} dari {Array.isArray(loanRecognitions) ? loanRecognitions.length : 0} data
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Effective Interest Rate Recognition Data</CardTitle>
                        <CardDescription>
                            Data pengakuan pinjaman dengan perhitungan Effective Interest Rate
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {!Array.isArray(loanRecognitions) ? (
                            <div className="text-center py-8">
                                <p className="text-muted-foreground">Error: Data tidak tersedia atau format tidak valid</p>
                                <p className="text-sm text-muted-foreground mt-2">
                                    Pastikan controller mengirimkan data 'loanRecognitions' sebagai array
                                </p>
                            </div>
                        ) : (
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead>EIR Calculated Convertion</TableHead>
                                            <TableHead>EIR Calculated Transaction Cost</TableHead>
                                            <TableHead>EIR Calculated UpFront Fee</TableHead>
                                            <TableHead>Outstanding Amount</TableHead>
                                            <TableHead>Outstanding Amount Initial Transaction Cost</TableHead>
                                            <TableHead>Outstanding Amount Initial UpFront Fee</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {filteredData.length > 0 ? (
                                            filteredData.map((item, index) => (
                                                <TableRow key={item.id || index}>
                                                    ...
                                                    <TableCell>{item.debitor_name || '-'}</TableCell>
                                                    ...
                                                    <TableCell>Rp {Number(item.original_balance || 0).toLocaleString('id-ID')}</TableCell>
                                                    ...
                                                    <TableCell>Rp {Number(item.eir_calculated_convertion || 0).toLocaleString('id-ID')}</TableCell>
                                                    <TableCell>Rp {Number(item.eir_calculated_transaction_cost || 0).toLocaleString('id-ID')}</TableCell>
                                                    <TableCell>Rp {Number(item.eir_calculated_up_front_fee || 0).toLocaleString('id-ID')}</TableCell>
                                                    <TableCell>Rp {Number(item.outstanding_amount || 0).toLocaleString('id-ID')}</TableCell>
                                                    <TableCell>Rp {Number(item.outstanding_amount_initial_transaction_cost || 0).toLocaleString('id-ID')}</TableCell>
                                                    <TableCell>Rp {Number(item.outstanding_amount_initial_up_front_fee || 0).toLocaleString('id-ID')}</TableCell>
                                                </TableRow>
                                            ))
                                        ) : (
                                            <TableRow>
                                                <TableCell colSpan={23} className="text-center py-8 text-muted-foreground">
                                                    {Array.isArray(loanRecognitions) && loanRecognitions.length === 0 
