...
export default function ReportJournalSecurities() {
    ...
    const filteredData = useMemo(() => {
        // Pastikan reportJournals adalah array
        if (!Array.isArray(reportJournals)) {
            console.warn('reportJournals is not an array:', reportJournals);
            return [];
        }
        
        return reportJournals.filter(item => {
            // Pastikan item dan posting_date ada
            if (!item || !item.posting_date) {
                return false;
            }
            
            try {
                // Assuming posting_date is in format YYYY-MM-DD or similar
                ...
                console.warn('Error parsing date:', item.posting_date, error);
                return false;
            }
        });
    }, [reportJournals, filters]);
    ...
    const exportToExcel = () => {
        ...
        loadXLSX()
            .then((XLSX) => {
                ...
                XLSX.utils.book_append_sheet(wb, ws, 'Report Journal Securities');
                
                // Generate filename
                const filename = `report_journal_securities_${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    const exportToPDF = () => {
        ...
        const htmlContent = `
            ...
            <html>
            <head>
                <title>Report Journal Securities</title>
                <style>
            ...
            <body>
                <div class="header-info">
                    <h1>Report Securities - Journal Securities</h1>
                        ...
                        ${filteredData.length > 0 ? 
                            ...
                            `<tr>
                                <td colspan="7" style="text-align: center; padding: 20px;">
                                    Data tidak ditemukan atau belum di-generate
                                </td>
                            </tr>`
                        }
                    </tbody>
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Report Securities - Journal Securities" />
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                {/* Header dengan judul dan tombol aksi */}
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Report Securities - Journal Securities</h1>
                        <p className="text-muted-foreground">
                            Jurnal transaksi sekuritas
                        </p>
                    </div>
                    ...
                    <CardContent className="pt-6">
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="text-sm text-muted-foreground">
                                {filteredData.length} dari {Array.isArray(reportJournals) ? reportJournals.length : 0} data
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Journal Securities</CardTitle>
                        <CardDescription>
                            Jurnal transaksi sekuritas
                        </CardDescription>
                    </CardHeader>
                        ...
                        {!Array.isArray(reportJournals) ? (
                            <div className="text-center py-8">
                                ...
                                <p className="text-sm text-muted-foreground mt-2">
                                    Pastikan controller mengirimkan data 'reportJournals' sebagai array
                                </p>
                            </div>
                        ) : (
