...
export default function ReportEvaluationTreasuryBond() {
    ...
    const filteredData = useMemo(() => {
        ...
        return reportSecurities.filter(item => {
            // Pastikan item dan evaluation_date ada
            if (!item || !item.evaluation_date) {
                return false;
            }
            
            try {
                // Assuming evaluation_date is in format YYYY-MM-DD or similar
                ...
                console.warn('Error parsing date:', item.evaluation_date, error);
                return false;
            }
        });
            ...
            .then((XLSX) => {
                ...
                const excelData = [
                    ...
                    ['No', 'Branch Number', 'Account Number', 'Bond Id', 'Issuer Name', 
                     'GL Account', 'Bond Type', 'GL Group', 'Evaluation Date', 'Maturity Date',
                     'Yield (YTM)', 'Face Value', 'Price', 'Mark To Market(MTM)', 'Carrying Amount',
                     'Unreleazed Gain/(Losses)', 'Cummulative Unreleazed Gain/(Losses)'],
                    // Data rows
                    ...filteredData.map((item, index) => [
                        ...
                        Number(item.unreleazed_gain_losses || 0),
                        Number(item.cummulative_unreleazed_gain_losses || 0)
                    ])
                ];
                ...
                ws['!cols'] = [
                    ...
                    { width: 20 },  // Unreleazed Gain/(Losses)
                    { width: 25 },  // Cummulative Unreleazed Gain/(Losses)
                ];
                ...
                XLSX.utils.book_append_sheet(wb, ws, 'Report Evaluation Treasury Bond');
                
                // Generate filename
                const filename = `report_evaluation_treasury_bond_${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    const exportToPDF = () => {
        ...
        const htmlContent = `
            ...
            <html>
            <head>
                <title>Report Evaluation Treasury Bond</title>
                <style>
            ...
            <body>
                <div class="header-info">
                    <h1>Report Securities - Evaluation Treasury Bond</h1>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>Unreleazed Gain/(Losses)</th>
                            <th>Cummulative Unreleazed Gain/(Losses)</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${filteredData.map((item, index) => `
                            <tr>
                                ...
                                <td class="amount">Rp ${Number(item.unreleazed_gain_losses || 0).toLocaleString('id-ID')}</td>
                                <td class="amount">Rp ${Number(item.cummulative_unreleazed_gain_losses || 0).toLocaleString('id-ID')}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Report Securities - Evaluation Treasury Bond" />
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                {/* Header dengan judul dan tombol aksi */}
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Report Securities - Evaluation Treasury Bond</h1>
                        <p className="text-muted-foreground">
                            Evaluasi Treasury Bond
                        </p>
                    </div>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Evaluation Treasury Bond</CardTitle>
                        <CardDescription>
                            Data evaluasi Treasury Bond dengan perhitungan Mark to Market
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead>Unreleazed Gain/(Losses)</TableHead>
                                            <TableHead>Cummulative Unreleazed Gain/(Losses)</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {filteredData.length > 0 ? (
                                            filteredData.map((item, index) => (
                                                <TableRow key={item.id || index}>
                                                    ...
                                                    <TableCell>{item.evaluation_date || '-'}</TableCell>
                                                    ...
                                                    <TableCell>Rp {Number(item.unreleazed_gain_losses || 0).toLocaleString('id-ID')}</TableCell>
                                                    <TableCell>Rp {Number(item.cummulative_unreleazed_gain_losses || 0).toLocaleString('id-ID')}</TableCell>
                                                </TableRow>
                                            ))
                                        ) : (
                                            <TableRow>
